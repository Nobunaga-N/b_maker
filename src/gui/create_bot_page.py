...
class ImageSearchModuleDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        hbox.addWidget(QLabel("Изображение:"))
        ...
        hbox2.addWidget(QLabel("Таймаут (ожидание появления изображения):"))
        ...
        hbox_add.addWidget(self.additional_image)
        ...
        image_layout.addWidget(QLabel("Список изображений для поиска:"))
        ...
        if_result_layout.addWidget(QLabel("Действия, если изображение найдено:"))
        self.log_event_if_found = QLineEdit()
        self.log_event_if_found.setPlaceholderText("Сообщение для вывода в консоль")
        if_result_layout.addWidget(QLabel("Сообщение в консоль:"))
        ...
        if_not_layout.addWidget(QLabel("Действия, если изображение не найдено:"))
        self.log_event_if_not_found = QLineEdit()
        self.log_event_if_not_found.setPlaceholderText("Сообщение для вывода в консоль")
        if_not_layout.addWidget(QLabel("Сообщение в консоль:"))
...
class SwipeModuleDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        hbox_start_x.addWidget(QLabel("Координата X:"))
        ...
        hbox_start_y.addWidget(QLabel("Координата Y:"))
        ...
        self.description_input.setPlaceholderText("Описание свайпа для отображения на холсте")
        ...
        hbox_sleep.addWidget(QLabel("Задержка после свайпа:"))
        hbox_sleep.addWidget(self.sleep_input)
        layout.addLayout(hbox_sleep)
...
class ActivityModuleDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        settings_layout.addWidget(QLabel("Действие при вылете игры:"))
...
class CreateBotPage(QWidget):
    ...
    def add_module_to_table(self, module_type: str, description: str, data: Dict[str, Any]):
        ...
        edit_btn = QPushButton("Изменить")
        edit_btn.clicked.connect(lambda: self.edit_module(row))
        actions_layout.addWidget(edit_btn)
    ...
    def edit_module(self, row: int):
        ...
        if module_type == "Клик":
            ...
            data = module.data
            dialog.x_input.setText(str(data["x"]))
            dialog.y_input.setText(str(data["y"]))
            dialog.description_input.setText(data["description"])
            dialog.console_description_input.setText(data["console_description"])
            dialog.sleep_input.setText(str(data["sleep"]))
            ...
            if dialog.exec():
                ...
                self.modules_table.item(row, 2).setText(new_description)
            ...
            # Заполняем диалог текущими данными
            data = module.data
            dialog.start_x_input.setValue(data["x1"])
            dialog.start_y_input.setValue(data["y1"])
            dialog.end_x_input.setValue(data["x2"])
            dialog.end_y_input.setValue(data["y2"])
            dialog.description_input.setText(data["description"])
            dialog.console_description_input.setText(data["console_description"])
            dialog.sleep_input.setValue(data["sleep"])
    ...
    def move_module_up(self):
        ...
        try:
            ...
            # Обновляем виджеты с кнопками
            current_widget = self.modules_table.cellWidget(current_row, 3)
            above_widget = self.modules_table.cellWidget(current_row - 1, 3)
            if current_widget and above_widget:
                self.modules_table.removeCellWidget(current_row, 3)
                self.modules_table.removeCellWidget(current_row - 1, 3)
                self.modules_table.setCellWidget(current_row - 1, 3, current_widget)
                self.modules_table.setCellWidget(current_row, 3, above_widget)
    ...
    def move_module_down(self):
        ...
        try:
            ...
            # Обновляем виджеты с кнопками
            current_widget = self.modules_table.cellWidget(current_row, 3)
            below_widget = self.modules_table.cellWidget(current_row + 1, 3)
            if current_widget and below_widget:
                self.modules_table.removeCellWidget(current_row, 3)
                self.modules_table.removeCellWidget(current_row + 1, 3)
                self.modules_table.setCellWidget(current_row + 1, 3, current_widget)
                self.modules_table.setCellWidget(current_row, 3, below_widget)
